...
void meanBlurConvolutionGPU(const float* in, float* out, const int w, const int h,int const kSize){
    ...
    err = cudaMemcpy(d_kernel, meanKernel.kdata.data(), kSize*kSize*sizeof(float), cudaMemcpyHostToDevice);
    if (err != cudaSuccess) { 
        std::cerr<<"cudaMemcpy d_kernel failed\n"; 
        cudaFree(d_in); 
        cudaFree(d_out); 
...
void laplacianConvolutionGPU(const float* in, float* out, const int w, const int h){
    ...
    err = cudaMemcpy(d_kernel, laplacianKernel.kdata.data(), kSize*kSize*sizeof(float), cudaMemcpyHostToDevice);
    if (err != cudaSuccess) { 
        std::cerr<<"cudaMemcpy d_kernel failed\n"; 
        cudaFree(d_in); 
        cudaFree(d_out); 
...
void gaussianConvolutionWithSharedGPU(const float* in, float* out, const int w, const int h, const int kSize, const float sigma) {
    ...
    err = cudaMemcpyToSymbol(constkernel, gaKernel.kdata.data(), kSize*kSize*sizeof(float), 0, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) { 
        std::cerr<<"cudaMemcpy constkernel failed\n"; 
        cudaFree(d_in); 
        cudaFree(d_out); 
    }
    dim3 block(16,16);
...
void sharpenConvolutionWithSharedGPU(const float* in, float* out, const int w, const int h){
    ...
    err = cudaMemcpyToSymbol(constkernel, sharpenKernel.kdata.data(), kSize*kSize*sizeof(float), 0, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) { 
        std::cerr<<"cudaMemcpy constkernel failed\n"; 
        cudaFree(d_in); 
        cudaFree(d_out); 
...
void meanBlurConvolutionWithSharedGPU(const float* in, float* out, const int w, const int h,int const kSize){
    ...
    err = cudaMemcpyToSymbol(constkernel, meanKernel.kdata.data(), kSize*kSize*sizeof(float), 0, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) { 
        std::cerr<<"cudaMemcpy constkernel failed\n"; 
        cudaFree(d_in); 
        cudaFree(d_out); 
...
void laplacianConvolutionWithSharedGPU(const float* in, float* out, const int w, const int h){
    ...
    err = cudaMemcpyToSymbol(constkernel, laplacianKernel.kdata.data(), kSize*kSize*sizeof(float), 0, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) { 
        std::cerr<<"cudaMemcpy constkernel failed\n"; 
        cudaFree(d_in); 
        cudaFree(d_out); 
