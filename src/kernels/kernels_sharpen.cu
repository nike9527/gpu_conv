...
__global__ void sharpenConvolutionGlobal(const float *__restrict__ input, float *__restrict__ output,
...
__global__ void sharpenConvolutionShared(const float *__restrict__ input, float *__restrict__ output,
...
void launchsharpen(filter_pipeline &pipe, const float *in, float *out, mem_type type, int block_w, int block_h)
{
    filter sharepenObj = filter::sharpen();
    const int width = pipe.width;
    const int height = pipe.height;
    const int ksize = sharepenObj.size;
    const int radius = sharepenObj.radius;
    ...
    if (type == mem_type::GLOBAL)
    {
        ...
        d_kernel.copy_from_host_async(sharepenObj.kdata.data(), ksize * ksize, pipe.stream.get());
        // start.record();
        sharpenConvolutionGlobal<<<grid, block, 0, pipe.stream.get()>>>(pipe.d_input.data(), pipe.d_output.data(), width, height, d_kernel.data(), ksize);
        // stop.record();
        // cudaEventSynchronize(stop);// 等待事件完成
    }
    else if (type == mem_type::SHAREDCONST)
    {
        CUDA_CHECK(cudaMemcpyToSymbolAsync(constkernel, sharepenObj.kdata.data(), ksize * ksize * sizeof(float), 0, cudaMemcpyHostToDevice, pipe.stream.get()));
        int shraedSize = (block_w + 2 * radius) * (block_h + 2 * radius) * sizeof(float);
        // start.record();
        sharpenConvolutionShared<<<grid, block, shraedSize, pipe.stream.get()>>>(pipe.d_input.data(), pipe.d_output.data(), width, height, ksize);
        // stop.record();
        // cudaEventSynchronize(stop);// 等待事件完成
    }
