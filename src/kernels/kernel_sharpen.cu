...
void kernel_sharpen::launch(const float* in,float* out,int w, int h, cuda_stream stream, const kernel_desc& desc,const filter& filter) {
    if(desc.dev_type == dev_type::DEVCPU){
        sharpenConvolutionCPU(in,out,w,h,filter);
    }else if(desc.dev_type == dev_type::DEVGPU && desc.mem_type == mem_type::GLOBAL){
        sharpenConvolutionGPU(in,out,w,h,filter);
    }else if(desc.dev_type == dev_type::DEVGPU && desc.mem_type == mem_type::SHAREDCONST){
        sharpenConvolutionWithSharedGPU(in,out,w,h,filter);
    }
}
/**
    * @brief  锐化滤波器(cpu omp)
    ...
    * @param h   宽度
    */
void kernel_sharpen::sharpenConvolutionCPU(const float* in, float* out, const int w, const int h, const filter& filter){
...
/**
    ...
    * @param ksize 内核大小
    */
void kernel_sharpen::sharpenConvolutionGPU(const float* in, float* out, const int w, const int h,const filter& filter, int block_w, int block_h){
    int kSize = filter.size;
...
/**
    ...
    * @param ksize 内核大小
    */
void kernel_sharpen::sharpenConvolutionWithSharedGPU(const float* in, float* out, const int w, const int h,const filter& filter,int block_w, int block_h){
    int kSize = filter.size;
    // cuda_event start, stop;
    cuda_memory<float> d_input(w * h);
    cuda_memory<float> d_output(w * h);
