...
__global__ void laplacianConvolutionGlobal(const float *__restrict__ input, float *__restrict__ output,
...
void launchLaplacian(filter_pipeline &pipe, const float *in, float *out, mem_type type, int block_w, int block_h)
{
    // cuda_event start, stop;
    filter laplacianObj = filter::laplacian();
    const int width = pipe.width;
    const int height = pipe.height;
    const int ksize = laplacianObj.size;
    const int radius = laplacianObj.radius;
    ...
    if (type == mem_type::GLOBAL)
    {
        ...
        d_kernel.copy_from_host_async(laplacianObj.kdata.data(), ksize * ksize, pipe.stream.get());
        // start.record();
        laplacianConvolutionGlobal<<<grid, block, 0, pipe.stream.get()>>>(pipe.d_input.data(), pipe.d_output.data(), width, height, d_kernel.data(), ksize);
        // stop.record();
        // cudaEventSynchronize(stop);// 等待事件完成
    }
    else if (type == mem_type::SHAREDCONST)
    {
        CUDA_CHECK(cudaMemcpyToSymbolAsync(constkernel, laplacianObj.kdata.data(), ksize * ksize * sizeof(float), 0, cudaMemcpyHostToDevice, pipe.stream.get()));
        int shraedSize = (block_w + 2 * radius) * (block_h + 2 * radius) * sizeof(float);
        // start.record();
        laplacianConvolutionShared<<<grid, block, shraedSize, pipe.stream.get()>>>(pipe.d_input.data(), pipe.d_output.data(), width, height, ksize);
        // stop.record();
        // cudaEventSynchronize(stop);// 等待事件完成
    }
