...
void kernel_meanBlur::launch(const float* in,float* out,int w, int h, cuda_stream stream, const kernel_desc& desc,const filter& filter) {
    if(desc.dev_type == dev_type::DEVCPU){
        meanBlurConvolutionCPU(in,out,w,h,filter);
    }else if(desc.dev_type == dev_type::DEVGPU && desc.mem_type == mem_type::GLOBAL){
        meanBlurConvolutionGPU(in,out,w,h,filter);
    }else if(desc.dev_type == dev_type::DEVGPU && desc.mem_type == mem_type::SHAREDCONST){
        meanBlurConvolutionWithSharedGPU(in,out,w,h,filter);
    }
}
...
/**
    ...
    * @param kSize  核大小
    */
void kernel_meanBlur::meanBlurConvolutionCPU(const float* in, float* out, const int w, const int h,const filter& filter){
...
/**
    ...
    * @param ksize 内核大小
    */
void kernel_meanBlur::meanBlurConvolutionGPU(const float* in, float* out, const int w, const int h,const filter& filter,int block_w, int block_h){
    int ksize = filter.size;
...
/**
    ...
    * @param ksize 内核大小
    */
void kernel_meanBlur::meanBlurConvolutionWithSharedGPU(const float* in, float* out, const int w, const int h,const filter& filter,int block_w, int block_h){
    int ksize = filter.size;
    // cuda_event start, stop;
    cuda_memory<float> d_input(w * h);
    cuda_memory<float> d_output(w * h);
