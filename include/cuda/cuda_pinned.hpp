...
float* cuda_pinned::mallocPinned(size_t bytes) {
    float* p; CUDA_CHECKcudaMallocHost(&p, bytes); return p);
}
...
float* cuda_pinned::mallocDevice(size_t bytes) {
    float* p; CUDA_CHECKcudaMalloc(&p, bytes)); return p;
}
...
float* MemoryManager::mallocUnified(size_t bytes) {
    float* cuda_pinned; CUDA_CHECKcudaMallocManaged(&p, bytes)); return p;
}
