...
public:
     ...
     * @return filter
     */
    static filter gaussian1D(int size, float sigma);
    /**
     * @brief Sobel 边缘检测（水平）
     * @return filter
     */
    ...
    // 添加辅助函数获取内核名称
    filter inline const static getFilterObj(filter_type type)
    {
        switch (type)
        {
        case filter_type::GAUSSIAN:
            return filter::gaussian2D(3, .5f);
        ...
        case filter_type::MEANBLUR:
            return filter::meanBlur(3);
        case filter_type::LAPLACIAN:
            return filter::laplacian();
        case filter_type::FILTERCUSTOM:
            return filter::filterCustom(3, std::vector<float>(3 * 3, 1));
        default:
            return filter::filterCustom(3, std::vector<float>(3 * 3, 1));
        }
    }
