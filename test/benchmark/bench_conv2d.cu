...
static void BM_Conv2D(benchmark::State &state)
{
    ...
    filter filterObj = filter::gaussian2D(ksize, sigma);
    cudaMemcpyToSymbolAsync(constkernel, filterObj.kdata.data(), ksize * ksize * sizeof(float), 0, cudaMemcpyHostToDevice, pipe.stream.get());
    int shraedSize = (block_w + 2 * filterObj.radius) * (block_h + 2 * filterObj.radius) * sizeof(float);
    // 预热（非常重要）
    for (int i = 0; i < 5; ++i)
    {
...
BENCHMARK(BM_Conv2D)->Args({1920, 1080, 3, 16, 16, 5})->Args({3840, 2160, 3, 16, 16, 5})->UseRealTime()->Unit(benchmark::kMillisecond);
